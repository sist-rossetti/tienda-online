-- Checkout server-side: calcula precios, descuento y stock en la base
-- en vez de confiar en lo que manda el cliente (Carrito.jsx).
--
-- Reemplaza el flujo anterior de:
--   insert directo en sales/sale_items + rpc decrement_stock + update coupons
-- por una sola función atómica (una función plpgsql = una transacción implícita).
--
...
begin
  ...
  insert into public.sales (
    order_number, customer_name, customer_email, customer_doc,
    total, discount, payment_method, status, note
  ) values (
    v_order_number, p_customer_name, nullif(p_customer_email, ''), nullif(p_customer_doc, ''),
...
grant execute on function public.checkout_order(text, text, text, text, text, text, jsonb) to anon, authenticated;
