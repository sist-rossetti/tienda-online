    ...
    <AdminLayout>
      <div className="p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-medium text-neutral-900 tracking-tight">Ventas</h1>
            <p className="text-sm text-neutral-400 mt-0.5">{totalOrdenes} órdenes en total</p>
          </div>
          <button
            onClick={() => {
              const csv = [
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            ...
            <div key={m.label} className="bg-white border border-neutral-200 rounded-2xl p-4">
              <p className="text-xs text-neutral-400 uppercase tracking-widest mb-1">{m.label}</p>
              <p className="text-xl font-medium text-neutral-900">{m.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-3 mb-6">
          <div className="flex-1 flex items-center gap-2 bg-white border border-neutral-200 rounded-full px-4 py-2.5">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por cliente u orden..." className="flex-1 text-sm text-neutral-900 outline-none" />
          </div>
          <select value={estadoFilter} onChange={e => setEstadoFilter(e.target.value)} className="border border-neutral-200 rounded-full px-4 py-2.5 text-sm text-neutral-600 bg-white outline-none">
          ...
          {loading ? (
            ...
            <div key={v.id} className="grid grid-cols-[90px_1fr_1fr_100px_90px_110px] px-5 py-4 border-t border-neutral-100 items-center hover:bg-neutral-50 transition cursor-pointer" onClick={() => setSelected(v)}>
              ...
              <div>
                ...
                <p className="text-xs text-neutral-400">{formatFecha(v.created_at)}</p>
              </div>
              ...
              <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                {ESTADOS.filter(s => s !== v.status).map(s => (
                  <button key={s} onClick={() => updateEstado(v.id, s)} className="text-xs text-neutral-400 hover:text-neutral-900 underline capitalize">{s}</button>
                ))}
              </div>
            </div>
          ))}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50,padding:'1rem'}}>
          <div className="bg-white rounded-3xl p-8 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-medium text-neutral-900 tracking-tight">Orden {selected.order_number}</h2>
              <button onClick={() => setSelected(null)} className="text-neutral-400 hover:text-neutral-900 text-xl">✕</button>
            </div>
            ...
            <div className="flex flex-col gap-2 mb-5">
              {[
                ...
                <div key={f.label} className="flex justify-between text-sm">
                  ...
                  <span className="text-neutral-900 font-medium">{f.value}</span>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-neutral-100 pt-4 mb-4">
              ...
              {selected.sale_items?.map(item => (
                <div key={item.id} className="flex justify-between text-sm py-2 border-b border-neutral-50">
                  <span className="text-neutral-900">{item.product_name} <span className="text-neutral-400">x{item.quantity}</span></span>
                  <span className="font-medium text-neutral-900">${Number(item.subtotal).toLocaleString('es-AR')}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-2 mb-3">
              {ESTADOS.map(s => (
                <button
                  ...
                  className={`flex-1 text-xs py-2 rounded-full font-medium transition border ${selected.status === s ? 'bg-neutral-900 text-white border-neutral-900' : 'border-neutral-200 text-neutral-500 hover:border-neutral-400'}`}
                >
                  {s}
                </button>
              ))}
