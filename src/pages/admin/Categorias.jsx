    ...
    <AdminLayout>
      <div className="p-4 sm:p-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
          <div>
            <h1 className="text-2xl font-medium text-stone-900 tracking-tight" style={{ fontFamily: 'Bricolage Grotesque' }}>Categorías</h1>
            <p className="text-sm text-stone-400 mt-0.5">{padres.length} categorías · {subcategorias.length} subcategorías</p>
          </div>
        ...
        <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden overflow-x-auto">
          ...
          <div className="grid grid-cols-[2fr_1fr_100px_120px] px-5 py-3 bg-stone-50 text-xs text-stone-400 uppercase tracking-widest">
          ...
          {loading ? (
            ...
            <div className="px-5 py-10 text-center text-sm text-stone-400">No hay categorías</div>
          ) : (
            <>
              {/* Categorías padre */}
              {padres.map(c => (
                <div key={c.id}>
                  <div className="grid grid-cols-[2fr_1fr_100px_120px] px-5 py-4 border-t border-stone-100 items-center">
                    <span className="text-sm font-medium text-stone-900">{c.name}</span>
                    <span className="text-sm text-stone-300">—</span>
                    <span>
                      <button onClick={() => toggleActive(c)} className={`text-xs px-3 py-1 rounded-full font-medium transition ${c.active ? 'bg-green-50 text-green-600 hover:bg-green-100' : 'bg-stone-100 text-stone-400 hover:bg-stone-200'}`}>
                  ...
                  {subcategorias.filter(s => s.parent_id === c.id).map(s => (
                    <div key={s.id} className="grid grid-cols-[2fr_1fr_100px_120px] px-5 py-3 border-t border-stone-50 items-center bg-stone-50/50">
                      <span className="text-sm text-stone-600 pl-5 flex items-center gap-2">
                        <span className="text-stone-300">└</span> {s.name}
                      </span>
                      <span className="text-sm text-stone-400">{c.name}</span>
                      <span>
                        <button onClick={() => toggleActive(s)} className={`text-xs px-3 py-1 rounded-full font-medium transition ${s.active ? 'bg-green-50 text-green-600 hover:bg-green-100' : 'bg-stone-100 text-stone-400 hover:bg-stone-200'}`}>
                          {s.active ? 'Activa' : 'Inactiva'}
                        </button>
                      </span>
                      <div className="flex gap-2">
                        <button onClick={() => openEdit(s)} className="w-8 h-8 border border-stone-200 rounded-xl flex items-center justify-center text-stone-400 hover:text-stone-900 hover:border-stone-400 transition text-xs">✏️</button>
                        <button onClick={() => { setDeleteError(''); setDeleteId(s.id) }} className="w-8 h-8 border border-stone-200 rounded-xl flex items-center justify-center text-stone-400 hover:text-red-500 hover:border-red-200 transition text-xs">🗑</button>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-sm">
            <h2 className="text-lg font-medium text-stone-900 tracking-tight mb-5">{editId ? 'Editar categoría' : 'Nueva categoría'}</h2>
            <div className="flex flex-col gap-4">
              ...
              <div>
                <label className="text-xs text-stone-400 uppercase tracking-widest mb-1.5 block">Categoría padre (opcional)</label>
                <select
                  value={parentId}
                  onChange={e => setParentId(e.target.value)}
                  className="w-full px-4 py-2.5 border border-stone-200 rounded-2xl text-sm outline-none focus:border-stone-400 transition bg-white"
                >
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-sm text-center">
            ...
            <h2 className="text-lg font-medium text-stone-900 mb-2">¿Eliminar categoría?</h2>
            <p className="text-sm text-stone-400 mb-4">Si tiene subcategorías, también se eliminarán.</p>
